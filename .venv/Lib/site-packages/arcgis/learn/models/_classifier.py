...
class FeatureClassifier(ArcGISModel):
    ...
    **Parameter**            **Description**
    ...
    @staticmethod
    def _satlas_resnet_backbones():
        """Supported list of satlas resnet backbones for this model."""
        resnet_keys = [r for r in hf_resnet_cfgs.keys() if "_satlas" in r]
        satlas_resnet_backbone = list(
            map(
                lambda m: "hf:" + m,
                resnet_keys,
            )
        )
        return satlas_resnet_backbone

    @staticmethod
    def _satlas_transformer_backbones():
        """Supported list of satlas transformer backbones for this model."""
        swin_keys = [
            attr
            for attr in dir(Swin_Weights)
            if not callable(getattr(Swin_Weights, attr)) and not attr.startswith("__")
        ]
        satlas_swin_backbone = list(
            map(
                lambda m: "hf:" + m,
                swin_keys,
            )
        )
        return satlas_swin_backbone
    ...
    def transformer_backbones():
        ...
        for tr in transformer_model_name:
            transformer_model.extend(
                timm.list_models(tr, pretrained=True, include_tags=False)
            )
        import fnmatch
        ...
        flt_models = []
        for f in flt:
            flt_models.extend(fnmatch.filter(transformer_model, f))
        transformer_model = sorted(set(transformer_model) - set(flt_models))
        ...
        satlas_transformer_backbones = FeatureClassifier._satlas_transformer_backbones()

        return transformer_model + satlas_transformer_backbones
    ...
    def satlas_backbones():
        """Supported list of satlas backbones for this model."""
        satlas_resnet_backbone = FeatureClassifier._satlas_resnet_backbones()
        satlas_transformer_backbone = FeatureClassifier._satlas_transformer_backbones()
        satlas_backbone = satlas_resnet_backbone + satlas_transformer_backbone
    ...
    def _supported_backbones():
        ...
        satlas_backbone = list(
            set(FeatureClassifier.satlas_backbones())
            - set(FeatureClassifier._satlas_transformer_backbones())
        )
