...
class _WarningSilencer:
    """
    Scoped silencer: suppress only specific {Deprecation,Future}Warning messages
    even if they are emitted deep inside importlib during module imports.
    """

    def __init__(self, patterns, categories):
    ...
    def __enter__(self):
        ...
        def _showwarning(msg, category, filename, lineno, file=None, line=None):
            if issubclass(category, self._cats):
                text = str(msg)
                if any(rx.search(text) for rx in self._regexes):
                    return
            return self._orig_showwarning(msg, category, filename, lineno, file, line)
