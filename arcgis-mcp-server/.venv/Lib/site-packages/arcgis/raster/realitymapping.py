...
class SensorType(Enum):
    ...
    | SATELLITE      | "Satellite"      |
    +----------------+------------------+
    | AERIAL_SCANNED | "AerialScanned"  |
    +----------------+------------------+
    | AERIAL_DIGITAL | "AerialDigital"  |
    +----------------+------------------+
    """
...
def _create_project(
    ...
    out_sr: Optional[Union[SpatialReference, dict]] = None,
    *,
    ...
    **Parameter**          **Description**
    ...
    scenario_type          Optional ScenarioType or string. The type of scenario for the imagery.
                           ...
                           The ScenarioType parameter is ignored if the sensor_type is
                           SensorType.SATELLITE or SensorType.AERIAL_SCANNED.

                           The default value is ScenarioType.DRONE.
    ...
    settings               Optional dictionary. The project definition dictionary.
                           The definition contains the template information such as adjustSettings,
    ...
    elif not isinstance(sensor_type, str) or not sensor_type in [
    ...
    sensor_scenario_map = {
        SensorType.DRONE.value: ScenarioType.DRONE.value,
        SensorType.SATELLITE.value: "",
        SensorType.AERIAL_SCANNED.value: "",
        SensorType.AERIAL_DIGITAL.value: ScenarioType.AERIAL_NADIR.value,
    }

    scenario_type = sensor_scenario_map[sensor_type]
    ...
    if sensor_type == SensorType.AERIAL_DIGITAL.value and scenario_type not in [
        ...
        raise RuntimeError(
            "Invalid scenario type for Aerial Digital sensor. Supported values are 'Aerial_Nadir', 'Aerial_Oblique'. Please refer to arcgis.raster.realitymapping.ScenarioType for more information."
        )

    if isinstance(out_sr, _arcgis.geometry.SpatialReference):
        out_sr = _json.loads(out_sr.JSON)
    if isinstance(out_sr, dict):
        if "wkid" not in out_sr:
            raise RuntimeError(
                "Invalid spatial reference dictionary. The dictionary must contain 'wkid' as key."
            )
    else:
        out_sr = {}
...
class Project:
    ...
    **Parameter**                            **Description**
    ...
    scenario_type                            Optional ScenarioType or string. The type of scenario for the imagery.
                                             ...
                                             The ScenarioType.AERIAL_NADIR and ScenarioType.AERIAL_OBLIQUE scenarios
                                             are only applicable for SensorType.AERIAL_DIGITAL sensor type.
                                             The ScenarioType.DRONE scenario is only applicable for SensorType.DRONE.
                                             This parameter is ignored if the sensor_type is SensorType.SATELLITE
                                             or SensorType.AERIAL_SCANNED.

                                             When the sensor_type is SensorType.DRONE, the default value is ScenarioType.DRONE.
                                             When the sensor_type is SensorType.AERIAL_DIGITAL, the default value is
                                             set to ScenarioType.AERIAL_NADIR.
    ...
    ------------------------------------     --------------------------------------------------------------------
    out_sr                                   Optional dictionary or :class:`~arcgis.geometry.SpatialReference`.
                                             The spatial reference to be used for this project.

                                             This can be specified as a well-known ID (WKID) and a VCS WKID.
                                             For e.g.: {"wkid": 4326, "vcsWkid": 4326}
