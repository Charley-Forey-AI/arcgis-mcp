...
import sys
import os
from .._utils.common import _WarningSilencer
import importlib

with _WarningSilencer(
    patterns=[
        r"`?TorchScript`? support for functional optimizers is deprecated",
        r"functools\.partial will be a method descriptor",
    ],
    categories=(DeprecationWarning, FutureWarning),
):
    # Preload all known noisy modules so they enter sys.modules quietly
    for mod in (
        "torch.distributed.optim",
        "torch.distributed.algorithms._optimizer_overlap.optimizer_overlap",
        "mmengine.optim.optimizer.zero_optimizer",
    ):
        try:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if use_abs_pos:
            ...
            if "prithvi" == self.backbone_name or "vit" in self.backbone_name:
                num_patches = (pretrain_img_size // patch_size) ** 2 + 1
                self.num_tokens = 1
