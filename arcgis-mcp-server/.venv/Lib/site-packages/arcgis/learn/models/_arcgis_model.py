...
try:
    ...
    from .._utils._timm_legacy_utils import get_timm_backbone_fn
...
def get_wavelengths_from_bandnames(band_names):
    ...
    for bandname in cleaned_bandnames:
        try:
            ...
            raise Exception(
                f'Band name "{cleaned2original_bandname_map[bandname]}" is not recognized and hence its wavelength cannot be inferred automatically. This backbone requires a list of central wavelengths corresponding to each data band (in micrometers). Please provide a value (list of floats) for the "wavelengths" keyword argument of this model. Example: for RGB bands, the wavelengths are [0.665, 0.56, 0.49] respectively.'
            )
    return wavelengths
...
def get_backbone_func(backbone, data, **kwargs):
    if backbone is None:
        ...
        if hasattr(models, backbone):
            ...
            bckbn = get_timm_backbone_fn(bckbn)
            if not bckbn is None:
                backbone = bckbn
        elif "hf:" in backbone:
            bckbn = backbone.split(":")[1]
...
class ArcGISModel(object):
    def __init__(self, data, backbone=None, **kwargs):
        ...
        if not getattr(data, "_is_empty", False) and hasattr(data, "_estimate_batch"):
            if data._estimate_batch:
                if self.__class__.__name__ not in unsupported_models:
                    ...
                    logger = logging.getLogger()
                    logger.warning(
                        "Batch size was set to None, but this model does not support "
                        "automatic batch size estimation. Using default batch_size=64 instead."
                    )
                    self._data.train_dl.batch_size = 64
                    self._data.valid_dl.batch_size = 64
    ...
    def _check_backbone_support(self, backbone):
        ...
        if type(backbone) is not str:
            if "timm" in backbone.__module__ or hasattr(backbone, "__is_timm_legacy__"):
                if hasattr(backbone, "__is_timm_tv__"):
                    backbone_name = "timm:tv_" + backbone.__name__
                else:
    ...
    def _create_emd_template(
        ...
        if self._backbone is None or type(self._backbone) is str:
            ...
            if self._backend == "tensorflow":
                ...
                if "timm" in self._backbone.__module__ or hasattr(
                    self._backbone, "__is_timm_legacy__"
                ):
                    if hasattr(self._backbone, "__is_timm_tv__"):
                        backbone = "timm:tv_" + self._backbone.__name__
                    else:
