...
class AutoML(object):
    ...
    **kwargs**
    ...
    def _impute_missing_values(self, data=None):
        ...
        for col in self._data._categorical_variables:
            data[col] = data[col].astype(object).replace({pd.NA: np.nan})
        try:
        ...
        if getattr(self._data, "_is_not_empty", True):
            try:
                with warnings.catch_warnings():
                    ...
                    # build a deepcopy of the dataframe to avoid changes to original dataframe
                    local_dataframe = self._data._dataframe.copy()
                    return self._model.score(
                        local_dataframe,
                        local_dataframe[self._data._dependent_variable],
                    )
            except:
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore", UserWarning)
                    # make a copy of the dataframe to avoid changes to original dataframe
                    local_dataframe = self._validation_data_df
                    return self._model.score(local_dataframe, val_labels)
    ...
    def from_model(cls, emd_path):
        ...
        **Parameter**            **Description**
        ...
        location_field = emd.get("location_field", [])
