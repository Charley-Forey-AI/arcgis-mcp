...
class ContentManager(object):
    ...
    def marketplace(self) -> "MarketPlaceManager":
        ...
        """
        if self._mrktplcmgr is None and self._gis._portal.is_arcgisonline is True:
    ...
    def can_delete(self, item: Item):
        """
        The ``can_delete`` method indicates whether an :class:`~arcgis.gis.Item` can be deleted or
        ...
        **Parameter**        **Description**
        ---------------     --------------------------------------------------------------------
        item                Required :class:`~arcgis.gis.Item`. The `Item` to be deleted.
        ===============     ====================================================================
        ...
        .. code-block:: python
            ...
            >>> gis = GIS(url="your_portal_url", username="your_portal_username", password="your_portal_password")
            >>> item = gis.content.get(itemid=<item_id>)
            >>> gis.content.can_delete(item=item)
            {"itemId": <item_id>, "success": True}
        ...
        **Status**          **Response**
        ...
        ---------------     --------------------------------------------------------------------
        failure             {"itemId": <item_id>,
                             "success": False,
                              "reason": {
                                "message": {
                                  "code": 400,
                                  "messageCode": "CONT_0048",
                                  "message": "Unable to delete item <item_id>. Delete protection is turned on.",
                                  "offendingItems": []
                                }
                              }
                             }
...
class ResourceManager(object):
    """
    This class is a helper class for managing resource files of an :class:`item <arcgis.gis.Item>`
    or :class:`user <arcgis.gis.User>`. An instance of this class is available as the
    *resource* property on each respective object. The methods on this class manage the
    resources of the *item* or *user*.

    .. note::
        Users do not create this class directly. Use the `resources` property on an *item*
        or *user* object to initialize the class instance.

    .. code-block:: python

        # Usage Example: Initializing object from an Item

        >>> from arcgis.gis import GIS
        >>> gis = GIS(profile="your_online_or_enterprise_profile")

        >>> water_item = gis.content.search(
        >>>                     query="Water Line *",
        >>>                     item_type="StoryMap"
        >>> )

        >>> sm_resource_mgr = water_item.resources
        >>> sm_resource_mgr

        <arcgis.gis.ResourceManager at 0x<mem_addr>>
    """
    ...
    def export(
        ...
        """
        Exports all resources as a zip file to the specified path.

        .. note ::
            Only supported for item resources.

        ===============     =====================================================
        **Parameter**       **Description**
        ---------------     -----------------------------------------------------
        save_path           Optional String. File path to save the zip file.
        ---------------     -----------------------------------------------------
        file_name           Optional String. Name to give to output zip file.
        ===============     =====================================================
    ...
    def add(
        ...
        """
        Adds new file resources to the resources of an *item* or *user*. For example,
        an image that is used as custom logo for a Report Template. All the files are
        added to the *resources* folder of the item. File resources use storage space
        from your quota and are scanned for viruses. For an item, the item size is
        updated to include the size of added resource files.
        ...
        * *ZIP*
        ...
        .. note::
            **Only** available to the item owner or an organization Administrator.
        ...
        **Parameter**      **Description**
        ...
        file              Optional string. The path to the file that needs to be added.
                          ...
                          .. note::
                              For a *user* object must be a binary file.
        ----------------  ---------------------------------------------------------------
        folder_name       Optional string. Provide a folder name if the file has to be
                          added to a folder under resources. **Only applicable to *item*
                          resources.
        ----------------  ---------------------------------------------------------------
        file_name         Optional string, **Required** on *user* resource manager. The
                          file name used to rename an existing file resource previously
                          uploaded, or to be used with *text* argument as file name.
        ----------------  ---------------------------------------------------------------
        text              Optional string. Text input to be added as a file resource,
                          used with *file_name* argument.

                          .. note::
                              If argument is provided on an *item* resource manager, For an *item*, if this
                              the *file_name* argument becomes **required.**
        ----------------  ---------------------------------------------------------------
        ...
        access            Optional String. Set file resource to be private regardless of
                          ...
                          Supported values for :class:`~arcgis.gis.Item` resources:

                          * `private`
                          * `inherit`.

                          Supported values for :class:`~arcgis.gis.User` resources:

                          * *userappprivate* - available only to the user through the app from
                            which the resource was uploaded.
                          * *allorgusersprivateapp* - available to all members of the organization
                            where the resource owner is part of and through the app where the resource was uploaded.
                          * *public* - available to everyone (including anonymous access) through any app.
                          * *userprivateallapps* - available through any app but only to the user that
                            uploaded the resource.`private`, `inherit`, or `public`.
        ----------------  ---------------------------------------------------------------
        ...
        * For :class:`~arcgis.gis.User` resources:
        ...
        * For :class:`~arcgis.gis.Item` resources:
            A Python dictionary in the following format (if successful):
        ...
        * For Both:
        ...
        .. code-block:: python
            ...
            >>> item.resources.add(
            >>>         file=r"/path/to/file",
            >>>         folder_name="soil_report_data",
            >>>         file_name="toxin_levels",
            >>>         access="private"
            >>> )
            ...
            >>> user.resources.add(
            >>>        file=r"C:path/to/file",
            >>>        file_name="aggregated_till_report"
            >>> )
        """
        if self._item:
    ...
    def update(
        ...
        """
        Updates the existing file resources of an *item*. File resources use
        storage space from your quota and are scanned for viruses. The *item* size
        ...
        * If an error occurs, information will be returned in the following format:
        ...
        .. code-block:: python
            ...
            >>> item = gis.content.get("<item_id>")
            >>> item.resources.add(
            >>>        file=r"/path/to/file.ext"
            >>>        folder_name="data_folder",
            >>>        file_name="org_banner.png"
            >>> )
    ...
    def list(self, as_list=True):
        """
        Provides a list of all file resources of an existing item.

        ================    ===============================================================
        **Parameter**       **Description**
        ----------------    ---------------------------------------------------------------
        as_list             Optional boolean.

                            * If *True*, returns a Python list of dictionaries
        ...
        * For item resources a Python list of dictionaries of the form:
        ...
        .. code-block:: python
            ...
            [
                {
                  "resource": "<resource1>",
                  "created": 1730852347000,
                  ...
                },
                {
                  "resource": "<resource2>"
                  "created": 1720752847000,
                  ...
                },
                ...
            ]
    ...
    def get(
        ...
        """
        Retrieves a specific resource of an existing *item* or *user*.

        .. note::
            **Only** available to the user, item owner, or an organization administrator.
        ...
        **Parameter**      **Description**
        ...
        :return:
           * Path to the downloaded file if getting a binary file (like a jpeg or png file) or if
             *try_json = False* when getting a JSON file.
    ...
    def remove(self, file: Optional[str] = None):
        """
        Removes a single resource file or all resources. For an :class:`~arcgis.gis.Item`,
        the item size is updated once resource files are deleted.

        .. note::
            **Only** available to the user, item owner, or organization Administrator.
        ...
        * If it fails, a dictionary with error information will be returned
...
class User(dict):
    ...
    **Property**             **Details**
    ...
    def resources(self) -> ResourceManager:
        """
        Returns a *ResourceManager* object for the :class:`~arcgis.gis.User` to .
        manage data, tools, or configurations available to the *user*.

        :return:
             A :class:`~arcgis.gis.ResourceManager` object.
        """
    ...
    def update(
        ...
        **Parameter**        **Description**
        ...
        ------------------  ----------------------------------------------------------
        tags                Optional string. Tags listed as comma-separated values, or a list of strings.
                            To remove tags, pass in an empty string or list.
        ------------------  ----------------------------------------------------------
...
class Item(dict):
    ...
    def resources(self) -> ResourceManager:
        """
        Returns a *ResourceManager* object for the :class:`~arcgis.gis.Item`
    ...
    def export(
        ...
        """
        This method exports an :class:`~arcgis.gis.Item` to the format specified
        in the *export_format* argument.

        .. note::
            **Only** the *item* owner or a user with an administrator role can run
            this method, unless a *Location Tracking Service* or *Location Tracking
            View* is used.

        This method is particularly useful for long running exports that could hold up
        a script since it can be run ansynchronously.
        ...
        **Parameter**        **Description**
        ...
        ---------------     --------------------------------------------------------------------
        export_format       Required string. The format to export the data to. Allowed values:
        ...
        parameters          Optional string. A JSON object describing the layers to be exported
                            and the export parameters for each layer. See the
                            `Export Item <https://developers.arcgis.com/rest/users-groups-and-items/export-item.htm>`_
                            in the REST API for guidance on structuring the object.

                            .. code-block:: python

                                # Usage Example: Parameters object entered as string argument

                                >>> import json
                                >>> from arcgis.gis import GIS

                                >>> flyr_item = gis.content.get("<feature_lyr_item_id>")
                                >>> for flyr in flyr_item.layers:
                                >>>    print(f"{flyr.properties.id:<5}{flyr.properties.name}")

                                0    city_streets
                                1    city_river_boundary
                                3    city_parks

                                >>> lyr_obj = {
                                >>>     "layers": [
                                >>>                {"id": 3, "designated_owner": "County Parks and Rec"}
                                >>>               ]
                                >>>    }

                                >>> lyr_str = json.dumps(lyr_obj)

                                >>> export_output = flyr_item.export(
                                >>>                        ....
                                >>>                     parameters = lyr_str
                                >>> )
        ---------------     --------------------------------------------------------------------
        ...
        ---------------     --------------------------------------------------------------------
        enforce_fld_vis     Optional boolean. Be default all attribute columns are exported.
                            If the operation is in a *view*, set this value to *True* to ensure
                            only the view's columns are exported. When the value is set to
                            *False*, all data and columns will be exported.
        ---------------     --------------------------------------------------------------------
        ...
        .. code-block:: python
            ...
            # Usage Example: Export Feature Layer item to csv item

            >>> from arcgis.gis import GIS
            >>> gis = GIS(profile="your_online_profile")

            >>> hurricane_item = gis.content.search(
            >>>                      query=f"Hurricane * AND owner:{gis.users.me.username}",
            >>>                      type="Feature Layer",
            >>>                      max_items=1
            >>> )[0]

            >>> hurrican_csv_item = hurricane_item.export(
            >>>                       title="hurricane_src_data_item",
            >>>                       export_format="CSV",
            >>>                       wait=True,
            >>>                       tags="Hurricane, Natural Disasters, source_content"
            >>>                     )
        """
        import time
